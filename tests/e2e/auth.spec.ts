...
test.describe("User Sign-up and Login", () => {
  ...
  test("should display signup errors", async ({ page }) => {
    ...
    await signUpPage.verifyTextAtPageBySelector(signUpPage.firstNameHelperText,'First Name is required')
    await signUpPage.typeLastName('Last')
    await signUpPage.clearField(signUpPage.signup_last_name_field)
    await signUpPage.verifyTextAtPageBySelector(signUpPage.lastNameHelperText,'Last Name is required')
    await signUpPage.typeUsername('User')
    await signUpPage.clearField(signUpPage.signup_username_field)
    await signUpPage.verifyTextAtPageBySelector(signUpPage.usernameHelperText,'Username is required')
    await signUpPage.typePassword('password')
    await signUpPage.clearField(signUpPage.signup_password_field)
    await signUpPage.verifyTextAtPageBySelector(signUpPage.passwordHelperText,'Enter your password')
  ...
  test("should error for an invalid user", async ({}) => {
    ...
    await loginPage.visualSnapshot("Sign In, Invalid Username and Password, Username or Password is Invalid");
  });
